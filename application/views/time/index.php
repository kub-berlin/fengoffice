...
<script>
    ...
    og.module_timeslots_grid = {
    	...
    	render_grid_actions: function(value, p, r) {
    		var actions = '';
    		if (r.id == 'quick_add_row' || r.data.id == '__total_row__' || r.data.id <= 0) return value;
...
<?php 
foreach ($additional_actions as $add_action) { ?>
...
<?php } ?>
    ...
    // filters
    var type_options = [
		[0, '<?php echo lang('all timeslots')?>'], [1, '<?php echo lang('task timeslots')?>'], [2, '<?php echo lang('time timeslots')?>']
    ];
    ...
    var filters = {
    	...
    	from_filter: {type:'date', label: '&nbsp;'+lang('from date')+': ',  value:'<?php echo array_var($current_filters,'from_filter')?>'},
    	to_filter: {type:'date', label: '&nbsp;'+lang('to date')+': ',  value:'<?php echo array_var($current_filters,'to_filter')?>'}
    };
